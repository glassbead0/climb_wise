...
def combine_proba_predictions_with_data(df, proba_prediction):
    ...
    combined_df['OS pred'] = proba_prediction[1][:, 0]
    combined_df['F pred'] = proba_prediction[1][:, 1]
    combined_df['RP pred'] = proba_prediction[1][:, 2]
    combined_df['F/H pred'] = proba_prediction[1][:, 3]
    ...
    return combined_df[['Route', 'RouteID', 'Date', 'Route Type', 'Alpine', 'Safety', 'Avg Stars', 'Pitches', 'Rating', 
                        'OS pred', 'F pred', 'RP pred', 'F/H pred', 'Lead Style',
...
def print_classification_report(y_test, y_pred_df):
    ...
    for i, resp in enumerate(y_test.columns):
        ...
        if resp == 'Lead Style':
            cr['Onsight'] = cr.pop('0')
            cr['Flash'] = cr.pop('1')
            cr['Redpoint'] = cr.pop('2')
            cr['Fell/Hung'] = cr.pop('3')
